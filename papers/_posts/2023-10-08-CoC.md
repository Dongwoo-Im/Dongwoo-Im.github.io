...
또한, appendix에서 CoC의 input format은 중요하지 않고, input이 continuous하지 않아도 문제없다고 주장한다. (실험 X)
...
- Point Reducer: k_neighbors만큼 fusion시키고, downsample_r만큼 point 수를 줄여나간다. (+ output dimension 조절)
- Context Cluster
