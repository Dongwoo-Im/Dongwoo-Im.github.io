...
그렇다면 왜 이미지 pixel이 아니라 token이냐? 이미지의 pixel을 그대로 사용하는 경우 메모리가 매우 많이 필요하고 (PixelCNN++에서 밝힌) high-frequency detail에 모델의 학습이 편향됩니다. 그래서 dVAE encoder로 이미지 token을 얻기 위해 2 stage 학습을 합니다. (dVAE -> prior)
...
저자들은 initial prior $p_{\psi}$가 codebook (K=8192)에 대한 uniform categorical 분포를 따른다고 설정하였습니다. 이러한 가정을 통해 ELBO 식에서 KL divergence를 계산할 수 있게 되고, 직관적으로는 codebook의 다양한 token이 학습 과정에서 선택되게 함으로써 codebook의 표현력을 높일 수 있었다고 생각합니다.

그리고 $q_{\phi}$는 32x32 grid 각각의 채널이 8192인 logit을 parameter로 하는 categorical 분포를 따른다고 설정하였습니다. 하지만 이렇게 설정하면 encoder는 discrete 분포를 modeling하게 되어 backpropagation이 어렵게 됩니다. 이러한 discrete 분포의 modeling 문제를 해결하고자 VQ-VAE에서는 VQ라는 straight-through estimator를 적용하였지만, dVAE에서는 gumbel-softmax relaxation을 적용했습니다.
