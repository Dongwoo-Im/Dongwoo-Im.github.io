...
참고로 이미지의 pixel을 그대로 사용하는 경우 메모리가 매우 많이 필요하고 (PixelCNN++에서 밝힌) high-frequency detail에 모델의 학습이 편향됩니다. 이를 해결하기 위해 전반적인 학습은 VQ-VAE와 같은 2 stage로 진행됩니다. (encoder를 통해 이미지를 token으로 mapping)
...
다음 수식은 생성 모델의 동작 방식과 objective가 될 ELBO 입니다.

- $p_{\theta,\psi}(x,y,z) = p_{\theta}(x|y,z) * p_{\psi}(y,z) $

- $ln(p_{\theta,\psi}(x,y)) \geq E(ln(p_{\theta}(x|y,z)) - \beta * D_{KL}(q_{\phi}(y,z|x), p_{\psi}(y,z))), z \sim q_{\phi}(z|x)$
- $x$ : image
...
추가로, dVAE decoder가 modeling하는 $p_{\theta}$의 likelihood는 log-laplace 분포를 사용하여 평가했습니다.

    - Appendix A.3. The Logit-Laplace Distribution
...
- Image token : dVAE-encoding, 1024 token, 8192 vocab size
    ...
    - ImageNet 실험 결과 overparameterized regime에서는 기존 sampling 방식이 useful regularizer의 기능을 했는데, DALL-E stage 2는 underparameterized regime여서 큰 상관 없을 것이라고 판단했다고 한다. (dataset과 model이 크기 때문에 underparameterized regime라고 판단한 것인가..?)
...
> 참고로, 이러한 sparse attention mask의 대전제는 **각 image token이 모든 text token에 attention 될 수 있는가** 였습니다.
...
각 예시에 대한 설명은 다음과 같으며, 아래 [Sparse Transformer](https://arxiv.org/abs/1904.10509)의 figure를 참고하시면 이해되실 것 같습니다.
...
- (a) on MS-COCO, (b) on CUB
    ...
    - 또한, x축은 blur kernel radius 입니다. 이러한 실험의 배경에는 DALL-E에서 dVAE의 image token을 처리함으로써, high frequency detail 보다는 low frequency information에 집중할 것이기 때문입니다. 즉, DALL-E는 애초에 high frequency detail을 학습하지 않았기 때문에, 생성된 이미지에 blur 처리를 하여 low frequency information 측면에서 다른 모델과 성능을 비교해보자는 아이디어라고 할 수 있습니다. 결과적으로 더 강한 blur가 적용될수록 다른 모델보다 DALL-E의 성능이 상대적으로 좋아지는 경향을 확인할 수 있습니다.
