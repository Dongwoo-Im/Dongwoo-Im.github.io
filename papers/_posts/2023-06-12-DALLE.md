...
저자들은 initial prior $p_{\psi}$가 codebook (K=8192)에 대한 uniform categorical 분포를 따른다고 설정하였습니다. 이러한 설정을 통해 ELBO 식에서 KL divergence를 계산할 수 있게 되고, 직관적으로는 codebook의 다양한 token이 학습 과정에서 선택되게 함으로써 codebook의 표현력을 높일 수 있었다고 생각합니다.
...
- (뇌피셜) DALL-E에서 VQ 대신에 gumbel-softmax를 사용한 이유는 T2I 모델이기 때문이라고 생각합니다. DALL-E에서는 text, image token을 바탕으로 예측된 image token을 모아 decoding하는데, VQ로 학습한 decoder로는 multi-modality가 반영된 token을 다루기 힘들지 않았을까 추측됩니다.

> 참고로, relaxation parameter인 $\tau$ 값이 0에 가깝게 작아질수록 one-hot vector와 가까워지고, 커질수록 uniform 분포와 가까워집니다.
...
자세한 과정은 `Appendix C. Details for Data Collection`, Conceptual Captions 논문, [DALLE-datasets](https://github.com/robvanvolt/DALLE-datasets) 레포를 참고하시면 도움되실 것 같습니다.
...
이외에도 `Appendix D. Guidelines for Mixed-Precision Training`에 적혀있는 다른 내용입니다.
...
Background로 알고 있어야 하는 내용은 [Sparse Transformer](https://arxiv.org/abs/1904.10509)의 `5.4. Saving memory by recomputing attention weights`입니다.

![sparse-fig4](https://dongwoo-im.github.io/assets/img/posts/2023-06-12-DALLE/sparse-fig4.webp){: .align-center}

간단히 요약하면, 메모리 복잡도가 제곱인 **self-attention**이나 hidden dim을 4배로 키우고 줄이는 **feed-forward network**처럼 연산 결과가 많은 메모리를 요구하는 경우, `gradient checkpointing`처럼 중간 결과만 저장하여 해당 연산을 recompute하는 것이 경제적이라는 내용입니다. 또한, 구현의 용이성을 위해 attention block의 dropout은 제거하였다고 하네요. 위 **figure 4**에서 어두운 음영 처리된 영역의 gradinet만 저장된다고 생각하시면 됩니다.

#### PowerSGD

다시 돌아오면, 12B parameter의 DALL-E 기준으로 16bit precision을 사용하더라도 24GB memory를 사용합니다. 이를 16GB V100에서 사용하기 위해 [ZeRO] 논문의 parameter sharding을 적용했다고 합니다. (읽어보지 않았지만, ZeRO 논문에서 parameter partitioning을 의미하는 것으로 추측됩니다.)

일반적으로 node(=machine) 내 통신보다 node 간 통신의 cost가 비싼 편인데, 저자들은 PowerSGD로 gradient를 압축함으로써 이 문제를 해결하고자 했습니다.

`Appendix E.1. Bandwidth Analysis`

`Appendix E.2. Implementation Details`
...
- Underflow를 피하기 위해 custom 16-bit format을 사용했다고 합니다. (on error buffers, their low-rank factors, all-reduce communication operations)
